...
**Mã nguồn chính:**
- [core.py](file:///d:/iCOMM/Legal-RAG/backend/retrieval/chunker/core.py) — Chunking FSM
- [metadata.py](file:///d:/iCOMM/Legal-RAG/backend/retrieval/chunker/metadata.py) — Regex Patterns & Helper Functions
- [relations.py](file:///d:/iCOMM/Legal-RAG/backend/retrieval/chunker/relations.py) — Ontology Relation Extraction
- [embedder.py](file:///d:/iCOMM/Legal-RAG/backend/retrieval/embedder.py) — Dense (REST API) + Sparse (BM25) Encoder
- [reranker.py](file:///d:/iCOMM/Legal-RAG/backend/retrieval/reranker.py) — Cross-Encoder Reranker (REST API)
- [hybrid_search.py](file:///d:/iCOMM/Legal-RAG/backend/retrieval/hybrid_search.py) — Tiered Prefetch + Rerank + Context Expansion
- [graph_db.py](file:///d:/iCOMM/Legal-RAG/backend/retrieval/graph_db.py) — Neo4j Graph Build & Retrieval Queries

**Notebook tham khảo (legacy):** [legal_rag_build_qdrant_2.ipynb](file:///d:/iCOMM/Legal-RAG/notebook/legal_rag_build_qdrant_2.ipynb)
...
Toàn bộ logic chunking nằm trong class `AdvancedLegalChunker` (file [core.py](file:///d:/iCOMM/Legal-RAG/backend/retrieval/chunker/core.py)), được thiết kế theo mô hình **Finite State Machine (FSM)** — máy trạng thái hữu hạn duyệt từng dòng văn bản từ trên xuống dưới. Regex patterns được tách ra module [metadata.py](file:///d:/iCOMM/Legal-RAG/backend/retrieval/chunker/metadata.py), logic bóc tách quan hệ nằm trong [relations.py](file:///d:/iCOMM/Legal-RAG/backend/retrieval/chunker/relations.py).
...
> [!NOTE]
> **Giải thích thuật toán FSM trong mã nguồn ([core.py](file:///d:/iCOMM/Legal-RAG/backend/retrieval/chunker/core.py)):**
...
Mở rộng từ Regex thuần, `AdvancedLegalChunker` sử dụng `extract_relationships_via_llm` (bóc AMENDS, REPLACES, REPEALS) và `extract_references_via_llm` (bóc Căn Cứ Pháp Lý) thông qua module `relations.py`. 

**Cơ chế Extraction logic:**
- **Trigger**: Khi Regex phát hiện các cụm từ khóa thay đổi hiệu lực (sửa đổi, bãi bỏ...), FSM sẽ khoanh vùng đoạn văn và gửi về `InternalLLMClient`.
- **LLM-Based Parsing**: Llama-3 8B nội bộ thực hiện bóc tách chính xác `doc_number`, `article`, `clause` bị tác động và trả về JSON chuẩn. 
- **Old Content Drill-down**: Riêng với trường hợp `AMENDS`, hệ thống sử dụng `extract_old_content_via_llm` để đào sâu vào văn bản cũ (kết hợp Boundary Regex và LLM) để trích xuất đúng nội dung nguyên bản của Điều/Khoản sắp bị thay đổi, lưu vào thuộc tính cạnh trong Neo4j.

Việc chạy 100% On-Premise cho phép quá trình trích xuất này diễn ra cực kỳ nhanh chóng và an toàn.

# Phần 2: Kỹ thuật dựng QdrantDB

## 2.1 Kiến trúc tổng quan

Mô hình Indexing dữ liệu RAG và Embed lưu trữ trên VectorDB (Qdrant) đi qua hệ thống ống rỗng thẳng đứng:
1. **Dữ liệu mẩu HF:** Trích bộ dữ liệu 90 Văn bản từ HuggingFace (phân mục Thuế, Thương mại, Xuất nhập khẩu).
2. **Parser lõi:** Băm cắt cấu trúc qua module class `AdvancedLegalChunker` sử dụng nhân xử lý đơn luồng CPU.
3. **Mã hoá nhúng:** Đi qua mô hình REST Adapter `InternalAPIEmbedder`. Hệ API cung cấp toạ độ nhúng không gian Dense 1024-d, kết hợp vector rỗng sơ khai cho luồng Sparse.
4. **Hạ điểm tập kết:** Vận tải thẳng Vector Data châm vào node Collection `legal_hybrid_rag_docs` của Qdrant đi kèm hành trang Metadata payload khổng lồ.

> [!NOTE]
> **Giải thích luồng Data đến Qdrant (`legal_rag_build_qdrant_2.ipynb`):**
> - **Bước 1 (Phân thân dữ liệu):** Lấy Dataset từ HuggingFace qua lọc Pandas, filter còn chuẩn 90 documents (30 nhóm "Thuế - Phí - Lệ Phí", 30 nhóm "Thương mại", và 30 nhóm "Xuất nhập khẩu"), đưa danh sách đi qua class `AdvancedLegalChunker` để cưa tách thành các chunk nhỏ.
> - **Bước 2 (Mã hoá Vector):** Bơm list string qua `InternalAPIEmbedder` gọi REST API tại `10.9.3.75:30010`. Kết quả sinh ra Dense Vector (1024 chiều tọa độ cosine không gian). Sparse Vector trả rỗng do API không hỗ trợ.
> - **Bước 3 (Cấu thành Payload):** Đóng gói UUID, Dense Vector và siêu dữ liệu `qdrant_metadata` thật gọn gàng vào Payload PointStruct (Loại bỏ các trường `neo4j_metadata` để tránh lãng phí RAM của VectorDB).
> - **Bước 4 (Upsert Song Song):** Bắn thẳng gói dữ liệu đã nén qua đường HTTP API để lưu lại trong Collection `legal_hybrid_rag_docs` trên Qdrant nội bộ. Lệnh sau gọi Cypher DB chèn Graph Node.

## 2.1.2 Kiến trúc Neo4j Graph DB (Dynamic Tree & Leaf Level Logic)

Đầu ra của Chunker chuyên trách đẩy `neo4j_metadata` vào file `graph_db.py`. Thiết kế sử dụng cơ chế lá đa dạng giải quyết độ sâu của mỗi văn bản.
...
Mã nguồn: [embedder.py](file:///d:/iCOMM/Legal-RAG/backend/retrieval/embedder.py)
...
Mã nguồn: [reranker.py](file:///d:/iCOMM/Legal-RAG/backend/retrieval/reranker.py)
...
> - **Bước 1 (Chia Batch):** Dòng loop While ngoài cùng phân chia mảng chứa 90 tài liệu làm các Batch nhỏ, quy định cứng ở ngưỡng `DOC_BATCH_SIZE = 250` (Tương đương 250 file trọn vẹn).
...
> - **Bước 3 (Encode):** `InternalAPIEmbedder` gọi REST API On-Premise để sinh Dense Vector Arrays.
> - **Bước 4 (Đẩy song song):** Tiêm thẳng batch Vector lên Qdrant thông qua module client SDK. Tiếp ngay sau đó, kéo array chunk kia ném vào hàm `build_neo4j()` kích hoạt query Cypher để đẩy lên GraphDB.
...
Quy trình xử lý hoàn chỉnh E2E tuân thủ thao tác lưu chuyển:

1. **DATA SOURCE:** Kết nối file JSON HF `vietnamese-legal-docs` fetch tập tài liệu.
2. **DATA LOADING:** Script pandas chuẩn hóa columns và cache.
3. **GLOBAL LOOKUPS:** Dựng Dictionary Indexing tốc độ lưu `meta_by_docnum_lookup` ~238K và text thô.
4. **CHUNKING:** FSM băm array Chunks độc lập rải rác thông qua logic cấp tiến lớp.
5. **EMBEDDING & PERSISTENCE:**
   - Vector Embedding REST tích Dense 1024-d và Local TF-IDF quăng vào DB lưu `Qdrant`.
   - Song song truyền tải payload siêu khổng lồ chui qua Driver chốt vào `Neo4j` xây dựng Dynamic Tree + Conflict Graph.

> [!NOTE]
> **Giải thích Tổng thể luồng E2E Pipeline RAG (`legal_rag_build_qdrant_2.ipynb`):**
> - **Bước 1 (Mạp Meta Dataset):** Tải trọn bộ DataFrame gốc từ HuggingFace `nhn309261/vietnamese-legal-docs`, lọc lấy đúng 90 văn bản (30 văn bản cho mỗi lĩnh vực Thuế, Thương mại, Xuất nhập khẩu).
...
> - **Bước 4 (Vector hóa Dense Layer):** 3,352 array string này gọi qua `InternalAPIEmbedder` (REST API tại `10.9.3.75:30010`).
> - **Bước 5 (Database Injecting Layer):** Tách 2 ngả đường. Đường mỏng đẩy JSON nhẹ nhàng qua Endpoint của Qdrant cho tốc độ search dưới 20ms. Mảng Metadata đồ sộ kèm viền Edge tham chiếu nhồi vào Cổng Driver Cypher của Neo4j Graph. Khép kín vòng đời tái chỉ mục RAG.
...
Khi user hỏi câu hỏi truy vấn, luồng tìm kiếm RAG đi qua các pha chuyển giao (mã nguồn: [hybrid_search.py](file:///d:/iCOMM/Legal-RAG/backend/retrieval/hybrid_search.py)):
