...
│   │   ├── chunker/                       # === ADVANCED LEGAL CHUNKER (Package) ===
│   │   │   ├── core.py                    # Vòng lặp Hierarchical Split
│   │   │   ├── metadata.py                # Regex Helpers (Chương, Điều, Phụ lục...)
│   │   │   ├── relations.py               # Trích xuất Quan hệ & Old/New Text
│   │   │   └── __init__.py                # Sinh Singleton (chunker = AdvancedLegalChunker())
│   │   ├── ingestion.py                   # Xử lý Upload (Trích Vector Qdrant + Ghép Graph Neo4j)
│   │   ├── graph_db.py                    # Điều phối Cypher Queries nạp Graph vào Neo4j
...
│       │   │   ├── ChatInput.tsx           # Input box + file upload
...
graph TD
    ...
    subgraph CHUNKER ["✂️ AdvancedLegalChunker (chunker/ Package)"]
        direction TB
        L1 ["Line-by-line Scanner"]
        L1 --> CH_DET {"Detect Structure / Format"}

        CH_DET -->|'Regex: Table (|...|)'| TBL ["📊 Table Trap<br/>in_table = True<br/>flush_table()"]
        CH_DET -->|'Regex: ^Chương/Phần'| CH ["📁 Chapter/Part<br/>current_chapter = 'Chương I...'"]
        CH_DET -->|'Regex: ^Điều/Mục'| ART ["📋 Article/Section<br/>current_article_ref = 'Điều 5'<br/>article_preamble = [title]"]
        CH_DET -->|'Regex: ^Khoản / 1.'| CL ["📝 Clause (Lvl 2)"]
        CH_DET -->|'Regex: ^a)'| PT ["📌 Point (Lvl 3)<br/>Append to active clause's points"]
        CH_DET -->|'Regex: ^PHỤ LỤC'| APP ["📎 Appendix Trap<br/>is_in_appendix = True<br/>3 Levels Pattern Matching"]

        PT --> FLUSH {"Smart Flush Limit"}
        FLUSH -->|'Size > 1200 chars'| FL1 ["Flush Early<br/>Keep Preamble + '(tiếp theo)'"]
        FLUSH -->|'Normal'| FL2 ["Continue Accumulating"]
    end

    subgraph OUTPUT ["📦 Split Payload Output"]
        direction TB
        QDRANT ["Qdrant Metadata (Semantic):
        • chunk_id, document_number
        • is_table, legal_sectors
        • breadcrumb"]

        NEO4J ["Neo4j Metadata (Graph):
        • chunk_index, document_id
        • title, legal_type, doc_status
        • effective_date, is_active, is_table
        • chapter/article/clause ref
        • amended_refs, replaced_refs
        • repealed_refs, basis_refs"]
...
Module `AdvancedLegalChunker` (Nằm trong Package `chunker/`) là bộ phân đoạn **hoàn toàn tự viết**, được thiết kế đặc thù cho cấu trúc văn bản pháp luật Việt Nam. Không dùng bất kỳ thư viện chunking có sẵn nào (LangChain TextSplitter, LlamaIndex SentenceSplitter, v.v.) vì chúng không hiểu cấu trúc phân cấp Chương → Mục → Điều → Khoản.

Kiến trúc package gồm 3 module nội bộ:
- `core.py`: Chứa class `AdvancedLegalChunker`, điều phối vòng lặp cắt văn bản.
- `metadata.py`: Chứa các hàm Regex helper làm sạch và nhận diện metadata.
- `relations.py`: Quản lý việc bóc tách Quan hệ đồ thị (`AMENDS`, `REPLACES`) và lưu vết *Old Text/New Text*.
...
```python
# Các regex patterns chính (chunker/metadata.py)
chapter_pattern  = r"(?im)^\s*(Chương|Phần)\s+([IVXLCDM0-9A-Z]+)\s*[\.\:\-]?\s*(.*)$"
article_pattern  = r"(?im)^\s*(Điều|Mục)\s+(\d+[A-Za-z0-9\/\-]*)\s*[\.\:\-]?\s*(.*)$"
clause_pattern   = r"(?im)^\s*(Khoản\s+\d+[\.\:\-]?)\s*(.*)$|^\s*(\d+(?:\.\d+)*[\.\)])\s*(.*)$|^\s*(\(\d+\))\s*(.*)$|^\s*([-+•])\s+(.*)$"
point_pattern    = r"(?im)^\s*([a-zđ]\s*[\)\.])\s*(.*)$"
appx_lvl1_pattern = r"(?im)^\s*([IVXLCDM]+|[A-Z])\s*[\.\:\-]\s*(.*)$"
```

**Quy tắc Flush Động cho Cấp Khoản/Điểm** (hàm `flush_article()` nội bộ): 
Quyết định khi nào nén gói chunk dựa vào kích thước linh hoạt của cửa sổ.

| Điều kiện | Ngưỡng | Hành vi |
|:---|:---|:---|
| Quá tải Khoản / Điểm | `Size > 1200` chars | **Flush sớm**. Flush tất cả đoạn đã thu thập. Giữ nguyên title/lời dẫn Khoản hiện tại và đính mác `(tiếp theo)` để bắt đầu nối chunk mới, không làm mất ngữ cảnh của những "Điểm" bơ vơ. |
| Bảng biểu | Regex đếm `\|` >= 2 | Set trạng thái `in_table = True`, kích hoạt module thu thập Header Bảng, tiến hành `flush_table` lưu Metadata riêng biệt. |

**Đặc thù thiết kế tiên tiến:**
- **Preamble Inheritance (v2)**: Khắc phục triệt để tình trạng một Điều luật dài nhiều trang giấy văng thành nhiều chunk làm mất Context mở đầu qua trick `(tiếp theo)`.
- **Table Detection**: Bắt chính xác vị trí ma trận/bảng biểu, gán Label `is_table` và tạo UUID Point riêng tránh làm nhiễu semantics của LLM.
- **Relation Extractor (amended/replaced/repealed)**: Trong phần Legal Basis Extraction, Chunker giờ quét cụm từ "(sửa đổi, bổ sung\|thay thế\|huỷ bỏ)...của Thông tư / Nghị định X". Phân rã thành dictionary để Neo4j tạo node Relation.
...
### 3.3 Bước Lưu Trữ Kép — Dual Storage (Qdrant + Neo4j)

Kiến trúc mới áp dụng mô hình **Dual Database RAG**, phân tách triệt để Payload đầu ra của Chunker chuyên trách cho đặc tính riêng của mỗi DB:

**A. Vector Store (Qdrant - Semantic Layer)**
Được "giảm cân" cực đoan (Light-weight) thu hẹp dung lượng để tập trung vào tốc độ quét Vector diện rộng (Similarity Matching).
- **Vectors**: Dense (1024d) + Sparse (Lexical weights từ BGE-M3).
- **`qdrant_payload`**: Chỉ giữ Hard Filter bắt buộc (`year`, `legal_sectors`, `is_table`) và chuỗi `breadcrumb` + `chunk_text`. 
- **Point ID**: String Unique ID (vd: `doc_id::article::c1`) map tĩnh tới Node Chunk Neo4j.

**B. Graph Store (Neo4j - Relational Layer)**
Hứng `neo4j_payload` siêu lớn từ `chunker.py` và chịu trách nhiệm định hình quan hệ:
- **Nodes**: `Document`, `Chunk`, `Sector`, `Authority`, `Signer`.
- **Edges cốt lõi**:
  - `(Chunk)-[:PART_OF]->(Document)`: Khắc phục điểm yếu "mất ngữ cảnh mẹ" của VectorRAG.
  - `(Document)-[:BELONGS_TO]->(Sector)`: Phục vụ luồng Sector Search.
  - `(Doc_A)-[:REFERENCES]->(Doc_B)`: Xây dựng từ `legal_basis_refs`. Phục vụ Multi-hop reasoning.
  - `(Doc_A)-[:AMENDS|REPLACES|REPEALS]->(Doc_B)`: Lưu trữ nội dung cũ `old_text` và mới `new_text` trực tiếp trên cạnh mũi tên (Edge properties). Thao tác nạp được khởi chạy tự động qua module `graph_db.py` khi có Upload file.

### 3.4 Bước Retrieval — `GraphAugmentedRetriever`

Quy trình truy xuất được nâng cấp từ Vector-only thành **Vector-to-Graph**:

1. **Semantic Search (Qdrant)**: Chạy Tiered Prefetch + RRF như cũ để tìm ra Top-K Chunk IDs có ngữ nghĩa sát nhất với câu hỏi.
2. **Graph Expansion (Neo4j)**: Lấy mảng Chunk IDs từ Qdrant bắn sang Neo4j qua Cypher query để thực hiện 2 thao tác:
   - **Thay thế Small-to-Big cũ**: Thay vì dùng Qdrant scroll, Neo4j gọi lệnh `MATCH (c:Chunk)-[:PART_OF]->(d:Document)<-[:PART_OF]-(siblings:Chunk) RETURN siblings` để gom toàn bộ Điều luật cực kỳ nhanh chóng.
   - **Multi-hop Reasoning**: Nếu câu hỏi phức tạp (QA Tình huống), Neo4j sẽ "đi dạo" (traverse) theo cạnh `[:REFERENCES]` để lôi các văn bản hướng dẫn/căn cứ pháp lý có liên quan đến Chunk ID vừa tìm được.
3. **Resolve References (Hybrid Search Extension)**: Truy vấn mở rộng Qdrant linh hoạt bằng tùy chọn Full-Text Match (`MatchText` thay cho `MatchValue`) khi đối soát `article_ref`. Hỗ trợ quét nhanh các trường hợp tham chiếu chéo (Cross-references) không đồng nhất định dạng.
4. **Neo4j GraphRAG (QdrantNeo4jRetriever)**: Function `graph_search()` trong `hybrid_search.py` cho phép 1 lệnh duy nhất nhảy từ Dense Search sang Node Traversal để kéo Context Đồ thị cực lớn cho Mode Xung đột.
5. **Context Assembly**: Ghép Text từ Qdrant và Relational Metadata từ Neo4j thành khối XML hoàn chỉnh giao cho LLM.
...
### 6.2 `QueryRouter` — Hệ thống Định tuyến 100% Agentic Intent (AUTO Mode)

**Mô tả chi tiết:** `QueryRouter` (`query_router.py`) là bộ phân loại ý đồ (intent classifier) định tuyến tự động **toàn bộ flow** của ứng dụng. Do Frontend không còn quản lý thủ công `ModeSelector` (luôn gửi `mode: 'AUTO'`), `QueryRouter` đóng vai trò "Gatekeeper", nhận câu hỏi gốc từ người dùng, đánh giá toàn diện và quyết định gọi chiến lược (Strategy) nào là hợp lý nhất.

- **Few-Shot Prompting**: 10 ví dụ mẫu trong system prompt, bao quát nhiều tình huống từ tư vấn chi tiết, tra cứu văn bản mảng, đến các trường hợp hỏi mông lung ("Chào bạn" → `GENERAL_CHAT`).
- **Model**: `LLM_ROUTING_MODEL` (`llama-3.1-8b-instant`) — model nhẹ, phản hồi cực kỳ tốc độ (~0.3s).
- **Output**: Trả về định dạng chuẩn JSON `{"intent": "LEGAL_QA|SECTOR_SEARCH|CONFLICT_ANALYZER|GENERAL_CHAT"}` kèm theo bộ thu thập metadata filters (nếu câu hỏi chứa năm, loại văn bản).
- **Fallback**: Tự chuyển sang `GENERAL_CHAT` nếu có rủi ro JSON Parsing, giúp hệ thống không bao giờ stuck.
- **Condense logic**: Nếu câu hỏi có dòng trao đổi lịch sử, tự động gọi `condense_question()` để chốt câu hỏi truy vấn ở dạng độc lập (VD: "Còn điều đó ở luật 2024 thì sao?").
...
> [!NOTE]
> **Tích hợp Neo4j cho luồng SECTOR_SEARCH:**
> Sức mạnh của đồ thị giúp luồng này không còn phụ thuộc vào việc khớp text cứng. Truy vấn Cypher `MATCH (d:Document)-[:BELONGS_TO]->(s:Sector {name: 'Thủ tục Tố tụng'})` sẽ trả về toàn bộ văn bản trong tích tắc. Bước "Coverage Bias Check" (tìm văn bản hướng dẫn) được giải quyết native bằng Neo4j thông qua việc quét các node lân cận.
...
> [!NOTE]
> **Tích hợp Neo4j cho luồng CONFLICT_ANALYZER:**
> GraphDB tỏa sáng mạnh mẽ nhất ở đây. Neo4j theo dõi các trạng thái `is_active` và tự động cập nhật các cạnh `[:AMENDS]` (Sửa đổi) hoặc `[:REPLACES]` (Thay thế). Khi Qdrant tìm thấy một mệnh đề, Neo4j sẽ báo động đỏ ngay lập tức nếu mệnh đề đó thuộc Node Document có kết nối với một văn bản mới hơn qua cạnh `[:REPLACES]`, triệt tiêu hoàn toàn rủi ro LLM sử dụng luật cũ (Lex Posterior).
...
| File | Vai trò |
|:---|:---|
| `contexts/ChatContext.tsx` | Global state quản lý toàn session: Tự động set hardcode `mode: AUTO` bắn xuống API, SSE consumer, file upload. Không còn tự quản lý State Manual Mode. |
| `components/chat/ChatArea.tsx` | Main chat area: markdown render, step indicators, reference cards |
| `components/chat/ChatInput.tsx` | Input box + file upload interaction |
