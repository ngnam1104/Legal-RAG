...
class RAGEngine:
    ...
    def chat(self, session_id: str, query: str, mode: str = "LEGAL_QA", file_path: str = None, top_k: int = 3, use_reflection: bool = True, use_rerank: bool = None) -> dict:
        """Thực thi luồng bằng LangGraph Orchestration."""
        ...
        print(f"\n" + "="*80)
        print(f"🚀 [LANGGRAPH ENGINE] CHAT REQUEST RECEIVED")
        ...
        try:
            ...
            answer = final_state.get("answer", "Xin lỗi, đã có lỗi xảy ra trong quá trình xử lý.")
            references = final_state.get("references", [])
            condensed_query = final_state.get("condensed_query", query)
            detected_mode = final_state.get("detected_mode")
